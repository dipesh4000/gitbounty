    ...
    <header className={`site-header${menuOpen ? " is-open" : ""}`} ref={headerRef}>
      <div className="wrap header-inner">
        ...
        <div className="header-cta">
          <button className="btn btn-ghost" type="button" hidden={Boolean(user)} onClick={() => connect("nav")}>Sign in</button>
          <button className="btn btn-primary" type="button" title={user ? "Sign out" : undefined} onClick={() => connect("nav")}>
            {user ? `@${user.github_login}` : "Connect GitHub"}
          </button>
        </div>
