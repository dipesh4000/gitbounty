...
@respx.mock
def test_owner_can_inspect_public_repository() -> None:
    ...
    with signed_in_client() as client:
        response = client.post("/api/maintainer/repositories/inspect", json={"repository": "https://github.com/aasha-malik/gitbounty"})

    assert response.status_code == 200
    assert response.json()["full_name"] == "aasha-malik/gitbounty"
    assert response.json()["open_issues"][0]["suggested_category"] == "backend"


@respx.mock
def test_user_without_write_access_cannot_publish_for_repository() -> None:
    payload = repo(owner_type="Organization", permissions={"admin": False, "maintain": False, "push": False})
    ...
    with signed_in_client() as client:
        response = client.post("/api/maintainer/repositories/inspect", json={"repository": "some-org/project"})
...
def test_publish_refetches_issue_and_writes_bounty(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with signed_in_client() as client:
        response = client.post(
            ...
            json={"repository": "aasha-malik/gitbounty", "issues": [{"number": 12, "points": 40, "category": "backend"}]},
        )

    assert response.status_code == 200
    assert response.json() == {"repository": "aasha-malik/gitbounty", "published": 1}
